...
/**
 * 场景7（扩展）：以用户身份删除文档（使用用户专属 token）
 */
@DeleteMapping("/users/{username}/datasets/{datasetId}/documents/{documentId}")
public void deleteDocumentAsUser(@PathVariable String username,
                                  @PathVariable String datasetId,
                                  @PathVariable String documentId,
                                  @RequestParam String email) {
    knowledgeDatasetService.deleteDocument(datasetId, documentId, username, email);
    System.out.println("文档已删除: " + documentId);
}
```
...
/**
 * 场景8（扩展）：以用户身份删除知识库（使用用户专属 token）
 */
@DeleteMapping("/users/{username}/datasets/{datasetId}")
public void deleteDatasetAsUser(@PathVariable String username,
                                 @PathVariable String datasetId,
                                 @RequestParam String email) {
    knowledgeDatasetService.deleteDataset(datasetId, username, email);
    System.out.println("知识库已删除: " + datasetId);
}
```
...
public class KnowledgeController {
    ...
    // 场景7（扩展）：以用户身份删除文档
    @DeleteMapping("/users/{username}/datasets/{datasetId}/documents/{documentId}")
    public ResponseEntity<Void> deleteDocumentAsUser(
    ...
    // 场景8（扩展）：以用户身份删除知识库
    @DeleteMapping("/users/{username}/datasets/{datasetId}")
    public ResponseEntity<Void> deleteDatasetAsUser(
            @PathVariable String username,
            @PathVariable String datasetId,
            @RequestParam String email) {
        knowledgeDatasetService.deleteDataset(datasetId, username, email);
